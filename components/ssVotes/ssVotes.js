...
export default function ssVotes() {
  ...
  const ssUpdated = () => {
    ...
    if (nfts?.length > 0) setToken(nfts[0] || { id: "" });

    if (nfts?.length > 0 && filteredAssets?.length > 0) {
  ...
  useEffect(() => {
    const vestVotesReturned = (vals) => {
      setVotes(
        vals.map((asset) => ({
          ...asset,
          value: BigNumber(asset.votePercent || 0).toNumber(0),
        }))
      );
    };
  ...
  const handleChange = (event) => {
    setToken(vestNFTs.find((nft) => nft.id === event.target.value));
    stores.dispatcher.dispatch({
      type: ACTIONS.GET_VEST_VOTES,
